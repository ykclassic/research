...
def test_reproducibility_hash_is_stable():
    ...
    assert first == second
    assert first == hashlib.sha256('{"dataset_version":"dataset-v1","engine_version":"engine-v3","feature_version":"features-v2","model_version":"model-v4","state":{"price":123.4,"regime":"TRENDING"}}'.encode()).hexdigest()


def test_api_key_authentication_uses_constant_time_hash_check(monkeypatch):
    raw="mrk_test-secret-12345"
    row={"id":"key-1","user_id":"user-1","key_hash":hashlib.sha256(raw.encode()).hexdigest(),"scopes":["market:read"],"active":True,"expires_at":None}
    ...
    result=infra.authenticate_api_key(raw)
...
def test_api_key_rejects_wrong_secret(monkeypatch):
    ...
    with pytest.raises(PermissionError,match="Invalid API key"):
        infra.authenticate_api_key("mrk_wrong-secret-12345")
