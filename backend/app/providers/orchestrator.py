...
class MarketDataOrchestrator:
    ...
    def _sync_usage(self, provider: MarketDataProvider) -> None:
        ...
        if provider.name == "twelve_data":
            self._twelve_data_quota.reconcile(
                provider_remaining=usage.credits_remaining,
    ...
    def _record_failure(self, provider: MarketDataProvider, error: str, latency_ms: int, error_code: ProviderErrorCode) -> None:
        with self._lock:
            state = self._state(provider)
            state.last_latency_ms = latency_ms
    ...
    def provider_status(self) -> list[ProviderStatus]:
        ...
        for provider in self.providers:
            state = self._state(provider)
            quote_budget = state.quote_budget_remaining
            ...
            statuses.append(ProviderStatus(
                ...
                last_error_code=state.last_error_code, credits_used=state.credits_used,
                credits_remaining=state.credits_remaining, usage_observed_at=state.usage_observed_at,
    ...
    def _semantic_quote(quote: Quote, symbol: str) -> Quote | None:
        ...
        if not is_market_open(symbol):
            return quote.model_copy(update={
                "status": QuoteStatus.MARKET_CLOSED, "market_open": False,
                "error": "Market is closed; provider returned the last validated quote.", "error_code": None,
            })
            ...
            try:
                ...
                for key in candidates:
                    ...
                    semantic = semantic.model_copy(update={
                        "latency_ms": latency_ms, "provider_attempts": tuple(diagnostics[key]),
                        "fallback_used": provider.name != self.providers[0].name or bool(excluded), "cache_hit": False,
                    })
                ...
                if semantic_success:
                    ...
                    self._record_failure(provider, detail, latency_ms, failures[0])
                ...
                if provider.name == "twelve_data":
                    # Reservations represent dispatched quote work. Provider usage is
                    # reconciled from response telemetry; unused reservation is not
                    # created because only the granted candidate slice is dispatched.
        ...
        for key in keys:
            ...
            if cached is not None:
                ...
                if quote.status == QuoteStatus.STALE or age > settings.quote_cache_seconds:
                    output[key] = quote.model_copy(update={
                        "status": QuoteStatus.STALE, "freshness_status": FreshnessStatus.STALE,
                        "freshness_age_seconds": max(quote.freshness_age_seconds or 0.0, age),
                        "error": "All live providers were unavailable; serving the last validated market quote.",
                        "error_code": ProviderErrorCode.ALL_PROVIDERS_UNAVAILABLE, "cache_hit": True,
                        "fallback_used": True, "provider_attempts": tuple(diagnostics[key]) or quote.provider_attempts, "latency_ms": 0,
                    })
                else:
            ...
            output[key] = Quote(symbol=key, provider_symbol=mapping.twelve_data, status=QuoteStatus.UNAVAILABLE, source=None,
                                error=f"{self._error_text(ProviderErrorCode.ALL_PROVIDERS_UNAVAILABLE)}: {detail}",
                                error_code=ProviderErrorCode.ALL_PROVIDERS_UNAVAILABLE, fallback_used=bool(diagnostics[key]),
    ...
    async def get_candles(self, symbol: str, timeframe: Timeframe, outputsize: int = 250, start_date: datetime | None = None, end_date: datetime | None = None, *, excluded_providers: set[str] | None = None) -> OHLCVDataset:
        ...
        for provider in self.providers:
            ...
            try:
                ...
                self._record_failure(provider, str(exc), int((time.perf_counter() - started) * 1000), classify_provider_error(exc))
