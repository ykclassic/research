...
class MarketDataOrchestrator:
    ...
    def __init__(self, providers: list[MarketDataProvider] | None = None) -> None:
        ...
        self._health: dict[str, ProviderHealthState] = {
            provider.name: ProviderHealthState() for provider in self.providers
        }
        self._lock = Lock()
        ...
        for provider in self.providers:
            ...
            results.append(
                ProviderStatus(
                    ...
                    message=(
                        "Configured; health is learned from real requests."
                        if configured
        ...
        if not force_refresh:
            ...
            if cached is not None:
                ...
                return cached_quote.model_copy(
                    update={
                        "cache_hit": True,
                        "provider_attempts": tuple(cached_quote.provider_attempts),
                        "latency_ms": 0,
                    }
                )

        for provider in self.providers:
            if provider.name in excluded or not provider.configured:
                continue
            state = self._state(provider)
            ...
            try:
                ...
                quote = quote.model_copy(
                    update={
                        ...
                        "fallback_used": provider.name != self.providers[0].name,
                        "cache_hit": False,
                    }
                )
                self.quote_cache.set(
                    key,
                    quote,
                    fresh_ttl_seconds=settings.quote_cache_seconds,
                    stale_ttl_seconds=settings.stale_quote_seconds,
                )
                return quote
            except (asyncio.TimeoutError, Exception) as exc:
                latency_ms = int((time.perf_counter() - started) * 1000)
                self._record_failure(provider, str(exc), latency_ms)

        cached = self.quote_cache.get(key, allow_stale=True)
        if cached is not None:
            stale_quote, age = cached
            return stale_quote.model_copy(
                update={
                    "status": QuoteStatus.STALE,
                    "freshness_status": FreshnessStatus.STALE,
                    "freshness_age_seconds": max(stale_quote.freshness_age_seconds or 0.0, age),
                    "cache_hit": True,
                    "fallback_used": True,
                    "provider_attempts": tuple(attempts) or stale_quote.provider_attempts,
        ...
        *,
        ...
        for provider in self.providers:
            ...
            try:
                ...
                self.candle_cache.set(
                    key,
                    dataset,
                    fresh_ttl_seconds=settings.quote_cache_seconds,
                    stale_ttl_seconds=settings.market_cache_stale_seconds,
                )
                return dataset
            except (asyncio.TimeoutError, Exception) as exc:
                latency_ms = int((time.perf_counter() - started) * 1000)
