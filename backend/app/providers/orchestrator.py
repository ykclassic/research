...
class MarketDataOrchestrator:
    ...
    async def get_quote(self, symbol: str, *, force_refresh: bool = False, excluded_providers: set[str] | None = None) -> Quote:
        ...
        if cached is not None and (not force_refresh or excluded):
            cached_quote, _ = cached
            return cached_quote.model_copy(update={
                "cache_hit": True,
                "fallback_used": bool(excluded),
                "provider_attempts": tuple(excluded) + cached_quote.provider_attempts if excluded else cached_quote.provider_attempts,
                "latency_ms": 0,
            })
        ...
        if cached is not None:
            ...
            return stale_quote.model_copy(update={
    ...
    async def get_candles(self, symbol: str, timeframe: Timeframe, outputsize: int = 250, start_date: datetime | None = None, end_date: datetime | None = None, *, excluded_providers: set[str] | None = None) -> OHLCVDataset:
        ...
        if cached is not None:
            ...
            return dataset.model_copy(update={
                "cache_hit": True,
                "fallback_used": bool(excluded),
                "provider_attempts": tuple(excluded) + dataset.provider_attempts if excluded else dataset.provider_attempts,
