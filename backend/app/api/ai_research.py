...
async def create_ai_research(request: AIResearchRequest) -> AIResearchResponse:
    ...
    try:
        ...
        if not analysis.data_quality.research_eligible:
            raise HTTPException(
                status_code=409,
                detail="AI research is disabled because the deterministic analysis dataset did not pass the research-eligibility gate.",
            )
        ...
        if analysis.latest_candle_timestamp != structure.latest_candle_timestamp:
            raise HTTPException(
                status_code=409,
                detail="AI research is disabled because the deterministic analysis and structure snapshots are not temporally aligned.",
            )
        if mtf.symbol != analysis.symbol or len(mtf.timeframes) != 4:
            raise HTTPException(
                status_code=409,
                detail="AI research is disabled because the deterministic multi-timeframe context is incomplete.",
            )
        ...
        context = {
            ...
            "evidence": [
                {
                    "id": "TA",
                    "type": "deterministic_technical_analysis",
                    "source": analysis.source,
                    "timeframe": analysis.timeframe.value,
                    "latest_candle_timestamp": analysis.latest_candle_timestamp,
                    "candle_count": analysis.candle_count,
                    "current_quote": analysis.current_quote.model_dump(mode="json"),
                    "indicators": analysis.indicators,
                    "data_quality": analysis.data_quality.model_dump(mode="json"),
                },
                {
                    "id": "REGIME",
                    "type": "deterministic_market_regime",
                    "source": regime.source,
                    "timeframe": regime.timeframe.value,
                    "latest_candle_timestamp": regime.latest_candle_timestamp,
                    "candle_count": regime.candle_count,
                    "regime": regime.regime.value,
                    "confidence": regime.confidence,
                    "evidence": regime.evidence.model_dump(mode="json"),
                    "thresholds": regime.thresholds.model_dump(mode="json"),
                    "rule_id": regime.rule_id,
                },
                {
                    "id": "STRUCTURE",
                    "type": "deterministic_market_structure",
                    "source": structure.source,
                    "timeframe": structure.timeframe.value,
                    "latest_candle_timestamp": structure.latest_candle_timestamp,
                    "candle_count": structure.candle_count,
                    "events": [event.model_dump(mode="json") for event in structure.events],
                },
                {
                    "id": "MTF",
                    "type": "deterministic_multi_timeframe_research",
                    "calculated_at": mtf.calculated_at,
                    "timeframes": [item.model_dump(mode="json") for item in mtf.timeframes],
                    "research": mtf.research.model_dump(mode="json"),
                },
            ],
        }
