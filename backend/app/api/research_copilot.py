...
async def run_copilot(request: CopilotRequest,user: UserResponse | None = Depends(get_current_user_or_github_actions),access_token: Annotated[str | None, Cookie(alias="mr_access_token")] = None) -> CopilotResponse:
    ...
    try:
        ...
        if interpretation["days"] > 7 or interpretation["intent"] in {"comparison", "historical_setup"}:
            require_feature(access_token, user.id, "copilot_deep_research"); consume_usage(access_token, user.id, "historical_queries", 1)
...
async def copilot_scheduler(x_research_copilot_secret: Annotated[str | None, Header()] = None) -> dict:
    ...
    try:
        ...
        webhooks = deliver_pending_webhooks(); exports = run_due_export_schedules(); maintenance = run_maintenance()
