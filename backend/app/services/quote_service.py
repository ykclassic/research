...
class QuoteService:
    ...
    async def get_quote(self, symbol: str, force_refresh: bool = False) -> Quote:
        ...
        return Quote(symbol=mapping.internal, provider_symbol=mapping.twelve_data, status=QuoteStatus.UNAVAILABLE, source="multi_provider", error="No configured provider returned a fresh quote.", cache_hit=False)
    ...
    async def _get_quote_bounded(self, symbol: str, force_refresh: bool) -> Quote:
        try:
            ...
            return Quote(symbol=mapping.internal, provider_symbol=mapping.twelve_data, status=QuoteStatus.UNAVAILABLE, source="multi_provider", error="Market-data providers exceeded the API latency budget.", cache_hit=False)
    ...
    async def provider_status(self) -> list[dict]:
        ...
        for provider in self.providers:
            ...
            results.append({"provider": provider.name, "configured": configured, "reachable": configured, "message": "Configured; live requests are available." if configured else "Not configured."})
        return results
