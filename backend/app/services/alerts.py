...
def create_rule(access_token: str, user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    symbol = normalize_symbol(str(payload["symbol"])).internal
        ...
        try:
            ...
            if key not in cache:
                ...
                try:
                    ...
                    regime = detect_regime(completed_dataset).regime.value
            ...
            if condition == "RSI_THRESHOLD" and rsi is not None:
                ...
                matches = _operator_matches(float(rsi), rule["operator"], threshold)
                active = matches and not bool(state.get("condition_active"))
                fingerprint = f"rsi:{observation_time.isoformat()}"
                detail = f"RSI(14) is {float(rsi):.2f} ({rule['operator']} {threshold:g})."
                state["condition_active"] = matches
            elif condition == "PRICE_CROSS":
                threshold = float(rule["threshold"])
                active = _crossed(float(state["last_price"]) if state.get("last_price") is not None else None, current_price, threshold, rule["operator"])
