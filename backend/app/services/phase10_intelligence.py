...
def create_chart_event(user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if event_type not in CHART_EVENT_TYPES:
        raise ValueError("Unsupported chart event type.")
    if not str(payload.get("symbol", "")).strip():
        raise ValueError("Symbol is required.")
    row = {
        "user_id": user_id,
        "symbol": str(payload["symbol"]).strip().upper(),
        "timeframe": str(payload.get("timeframe", "H1")).upper(),
        "event_type": event_type,
        "occurred_at": payload.get("occurred_at") or _now(),
        "price": payload.get("price"),
        "payload": payload.get("payload") or {},
        "source": payload.get("source"),
        "engine_version": payload.get("engine_version") or "phase10-chart-events-v1",
    }
    return service_request("POST", "research_chart_events", json=row, prefer="return=representation").json()[0]
...
def create_fundamental_observation(user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if observation_type not in FUNDAMENTAL_TYPES:
        raise ValueError("Unsupported fundamental observation type.")
    if not str(payload.get("symbol", "")).strip():
        raise ValueError("Symbol is required.")
    row = {
        "user_id": user_id,
    ...
    return service_request("POST", "fundamental_observations", json=row, prefer="return=representation").json()[0]
...
def create_model_version(user_id: str, model_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    model = _one("research_models", {"id": f"eq.{model_id}", "user_id": f"eq.{user_id}"})
    if not model:
    ...
    row = {
        ...
        "feature_version": str(payload["feature_version"]).strip(),
        "training_period_start": payload["training_period_start"],
        "training_period_end": payload["training_period_end"],
        "validation_period_start": payload["validation_period_start"],
        "validation_period_end": payload["validation_period_end"],
        ...
        "performance_by_regime": payload.get("performance_by_regime") or {},
        "degradation_monitoring": payload.get("degradation_monitoring") or {},
    }
...
def list_models(user_id: str) -> list[dict[str, Any]]:
    ...
    for model in models:
        model["versions"] = _rows("research_model_versions", {"select": "*", "model_id": f"eq.{model['id']}", "user_id": f"eq.{user_id}", "order": "created_at.desc"})
    return models
...
def evaluate_model_version(user_id: str, version_id: str, test_results: dict[str, Any], calibration: dict[str, Any], performance_by_regime: dict[str, Any], degradation_monitoring: dict[str, Any]) -> dict[str, Any]:
    ...
    status = "VALIDATING" if not passed else "VALIDATING"
    updated = service_request("PATCH", "research_model_versions", params={"id": f"eq.{version_id}", "user_id": f"eq.{user_id}"}, json={"status": status, "test_results": test_results, "calibration": calibration, "performance_by_regime": performance_by_regime, "degradation_monitoring": degradation_monitoring, "test_passed": passed}, prefer="return=representation").json()[0]
...
def promote_model_version(user_id: str, version_id: str) -> dict[str, Any]:
    version = _one("research_model_versions", {"id": f"eq.{version_id}", "user_id": f"eq.{user_id}"})
    if not version.get("test_passed"):
        raise ValueError("Model version cannot be promoted until validation records test_passed=true.")
    ...
    if active:
        service_request("PATCH", "research_model_versions", params={"id": f"eq.{active}", "user_id": f"eq.{user_id}"}, json={"status": "RETIRED"})
...
def rollback_model_version(user_id: str, version_id: str, reason: str) -> dict[str, Any]:
    ...
    if model.get("active_version_id") == version_id:
        service_request("PATCH", "research_models", params={"id": f"eq.{model['id']}", "user_id": f"eq.{user_id}"}, json={"active_version_id": None})
...
def overview(user_id: str) -> dict[str, Any]:
    ...
    return {
        "model_governance": {"models": len(models), "versions_by_status": statuses, "automatic_promotion": False},
        "advanced_charting": {"events": chart_count, "event_types": sorted(CHART_EVENT_TYPES)},
        "fundamental_intelligence": {"observations": fundamental_count, "types": sorted(FUNDAMENTAL_TYPES)},
        "product_intelligence": {"tracked_events": event_count},
        "optimization": {"cache_namespace": "phase10", "maintenance": "scheduler-ready"},
    }
...
def run_maintenance() -> dict[str, int]:
    ...
    for row in expired:
        service_request("DELETE", "research_cache_entries", params={"cache_key": f"eq.{row['cache_key']}"})
