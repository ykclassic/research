...
class ResearchReportService:
    ...
    def _validate_quote_policy(quote: Quote, config: ResolvedResearchConfiguration) -> None:
        ...
        if config.reject_stale_data and (quote.status == QuoteStatus.STALE or (age is not None and age > config.maximum_data_age_seconds)):
    ...
    def _validate_dataset_policy(dataset: OHLCVDataset, config: ResolvedResearchConfiguration) -> None:
        ...
        if age is None and dataset.provider_timestamp is not None:
            age = max(0.0, (datetime.now(timezone.utc) - dataset.provider_timestamp).total_seconds())
        if config.reject_stale_data and (dataset.freshness_status.value == "STALE" or (age is not None and age > config.maximum_data_age_seconds)):
    ...
    def _configuration_model(config: ResolvedResearchConfiguration) -> ResearchRequestConfiguration:
        return ResearchRequestConfiguration(default_asset=config.default_asset, default_asset_class=config.default_asset_class, default_timeframe=config.default_timeframe, analysis_depth=config.analysis_depth, **config.requested_components, maximum_data_age_seconds=config.maximum_data_age_seconds, reject_stale_data=config.reject_stale_data, require_completed_candles=config.require_completed_candles, allow_cached_data_fallback=config.allow_cached_data_fallback)
    ...
    async def generate(self, symbol: str | None = None, configuration: ResolvedResearchConfiguration | None = None) -> ResearchReport:
        ...
        if config.technical_analysis_enabled or config.market_structure_enabled or config.multi_timeframe_enabled:
            if config.multi_timeframe_enabled:
                ...
                if primary_timeframe not in required_timeframes: required_timeframes.append(primary_timeframe)
            else: required_timeframes = [primary_timeframe]
        ...
        if config.technical_analysis_enabled and primary is not None:
            if len(primary.completed_candles) < MIN_REPORT_CANDLES: raise ValueError(f"At least {MIN_REPORT_CANDLES} completed candles are required for technical research.")
        ...
        if config.market_structure_enabled and primary is not None:
            ...
            if not config.technical_analysis_enabled: support, resistance = self._support_resistance(primary)
        ...
        if config.technical_analysis_enabled:
            ...
            if isinstance(atr, (int, float)) and current_quote.price > 0: volatility = float(atr) / current_quote.price * 100
        ...
        if config.fundamental_analysis_enabled or config.news_analysis_enabled:
            ...
            except (RuntimeError, ValueError, asyncio.TimeoutError): pass
        ...
        interpretation = None
        if config.ai_interpretation_enabled: interpretation = f"{symbol} currently has a {status.trend.lower()} technical trend, {status.momentum.lower()} momentum, and a {status.market_regime} regime. The interpretation is assembled from the requested deterministic evidence and does not infer causation from headlines."
