...
create table if not exists public.fundamental_observations (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  symbol text not null,
  observation_type text not null check (observation_type in ('FINANCIAL_STATEMENT','EARNINGS','VALUATION','ANALYST_REVISION','CORPORATE_ACTION','FUNDAMENTAL_TREND')),
  period_start date,
  period_end date,
...
create table if not exists public.research_model_versions (
  id uuid primary key default gen_random_uuid(),
  model_id uuid not null references public.research_models(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  ...
  performance_by_regime jsonb not null default '{}'::jsonb,
  degradation_monitoring jsonb not null default '{}'::jsonb,
  test_passed boolean not null default false,
  promotion_actor_user_id uuid references auth.users(id),
  promoted_at timestamptz,
  rollback_reason text,
  created_at timestamptz not null default timezone('utc',now()),
  updated_at timestamptz not null default timezone('utc',now()),
  unique(model_id,version),
  check (training_period_start <= training_period_end),
  check (validation_period_start <= validation_period_end),
...
create table if not exists public.research_model_events (
  id uuid primary key default gen_random_uuid(),
  model_version_id uuid not null references public.research_model_versions(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  event_type text not null check (event_type in ('CREATED','VALIDATION_STARTED','EVALUATED','PROMOTED','ROLLED_BACK','RETIRED')),
