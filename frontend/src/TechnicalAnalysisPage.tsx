...
function value(data: TechnicalAnalysis | null, key: string, digits = 4): string {
  ...
  return typeof raw === "number" && Number.isFinite(raw)
    ? raw.toLocaleString(undefined, { maximumFractionDigits: digits })
    : "—";
}
...
export default function TechnicalAnalysisPage({
  ...
  user: User;
  onLogout: () => void;
  setPage: (page: "market" | "watchlists" | "analysis") => void;
}) {
  ...
  const range = useMemo(
    () => buildRange(rangePreset, new Date(), customStart, customEnd),
    [rangePreset, customStart, customEnd],
  );
  const rangeInvalid = rangePreset === "custom" && (
    (!customStart && !!customEnd)
    || (!!customStart && !customEnd)
    || (!!customStart && !!customEnd && customStart > customEnd)
  );
  const requestKey = useMemo(
    () => JSON.stringify({ symbol, timeframe, range }),
  ...
  const load = useCallback(async (force = false) => {
    if (rangeInvalid) {
      setError(customStart > customEnd
        ? "The range start must be on or before the range end."
        : "Select both a range start and range end before loading historical data.");
      return;
    }
  ...
  const trend = useMemo(
    () => typeof data?.indicators.trend === "string" ? data.indicators.trend : "UNKNOWN",
    [data],
  );
  const completedCandles = useMemo(
    () => data?.candles.filter(candle => candle.is_complete) ?? [],
    [data],
  );
    ...
    <div className="app">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="Research sections">
          ...
          <button className="nav-button active" onClick={() => setPage("analysis")}>Technical Analysis</button>
        </nav>
      ...
      <main>
        ...
        <section className="ta-controls panel">
          <div><label>Instrument<select value={symbol} onChange={e => setSymbol(e.target.value)} disabled={loading || refreshing}>{SYMBOLS.map(item => <option key={item}>{item}</option>)}</select></label></div>
          <div><label>Timeframe<select value={timeframe} onChange={e => setTimeframe(e.target.value)} disabled={loading || refreshing}>{TIMEFRAMES.map(item => <option key={item}>{item}</option>)}</select></label></div>
          <div><label>Historical range<select value={rangePreset} onChange={e => setRangePreset(e.target.value as RangePreset)} disabled={loading || refreshing}>
            <option value="recent">Recent API window</option><option value="1d">1 day</option><option value="1w">1 week</option><option value="1m">1 month</option><option value="3m">3 months</option><option value="custom">Custom</option>
          </select></label></div>
          <label className="chart-date-field">Start<input type="date" value={customStart} onChange={e => { setCustomStart(e.target.value); setRangePreset("custom"); }} disabled={loading || refreshing || rangePreset !== "custom"}/></label>
          <label className="chart-date-field">End<input type="date" value={customEnd} onChange={e => { setCustomEnd(e.target.value); setRangePreset("custom"); }} disabled={loading || refreshing || rangePreset !== "custom"}/></label>
          <label className="chart-auto-refresh"><input type="checkbox" checked={autoRefresh} onChange={e => setAutoRefresh(e.target.checked)}/> Auto-refresh (60s)</label>
          <button className="refresh" onClick={() => void load(true)} disabled={loading || refreshing || rangeInvalid}><RefreshCw size={16} className={refreshing ? "spin" : ""}/>{refreshing ? "Refreshing" : "Refresh"}</button>
        </section>
        ...
        {loading ? <div className="panel empty" role="status">Loading historical candles and indicators from the canonical API…</div> : !data && !error ? <div className="panel empty">No analysis is currently available.</div> : data && <>
          <section className="ta-overview">
            <div className="panel">
              ...
              <div className="ta-price">{quotePrice?.toLocaleString(undefined, { maximumFractionDigits: 8 }) ?? "—"}</div>
              <div className="panel-head"><span>Current quote · {currentQuote?.status ?? "UNKNOWN"} · {currentQuote?.source ?? "unknown"}</span><span>Observed: {formatTimestamp(currentQuote?.timestamp ?? undefined)}</span></div>
              <div className={`large-status ${indicatorClass(trend)}`}><span className="dot"/>{trend}</div>
              <div className="panel-head"><span>Last completed close: {lastCompleted?.close.toLocaleString(undefined, { maximumFractionDigits: 8 }) ?? "—"}</span><span>API calculated: {formatTimestamp(data.calculated_at)}</span></div>
            </div>
            <div className="panel">
              <div className="panel-head"><div><h3>Workspace state</h3><span>Range and refresh controls affect API requests only.</span></div><ShieldCheck size={20}/></div>
              <div className="metric-grid">
                <div><span>Data source</span><strong>{data.source}</strong></div>
                <div><span>Range</span><strong>{rangePreset === "recent" ? "Recent window" : rangePreset === "custom" ? "Custom" : rangePreset}</strong></div>
                <div><span>Completed candles</span><strong>{completedCandles.length}</strong></div>
                <div><span>Auto-refresh</span><strong>{autoRefresh ? "60s" : "Off"}</strong></div>
              </div>
            </div>
          </section>
