...
export type CompletenessStatus = "COMPLETE" | "PARTIAL" | "INVALID" | "UNKNOWN";
...
export interface RegimeEvidence {
  price: number; ema_50: number | null; ema_200: number | null;
  price_above_ema_200: boolean | null; ema_50_above_ema_200: boolean | null;
  adx: number | null; atr: number | null; atr_percent: number | null; atr_percentile: number | null;
  bb_width: number | null; bb_width_percentile: number | null; trend_direction: string;
  trend_persistence: number; directional_move_ratio: number;
}
export interface RegimeThresholds {
  adx_strong: number; persistence_strong: number; persistence_weak: number;
  directional_ratio_strong: number; directional_ratio_weak: number;
  volatility_high_percentile: number; volatility_low_percentile: number;
}
export interface RegimeResult {
  symbol: string; timeframe: string; source: string; calculated_at: string;
  provider_timestamp: string | null; latest_candle_timestamp: string; candle_count: number;
  regime: MarketRegime; confidence: number; evidence: RegimeEvidence; thresholds: RegimeThresholds;
  rule_id: string; rule: string;
  request_latency_ms?: number | null;
  freshness_status?: FreshnessStatus;
  freshness_age_seconds?: number | null;
  completeness_status?: CompletenessStatus;
...
async function requestCsrfToken(): Promise<string> {
  ...
  if (!response.ok) { let message = `Request failed: ${response.status}`; try { const body = await response.json() as { detail?: string }; if (body.detail) message = body.detail; } catch {} throw new ApiError(message, response.status); }
...
export async function getTechnicalAnalysis(symbol: string, timeframe = "1h", limit = 250, range: TechnicalAnalysisRange = {}): Promise<TechnicalAnalysis> {
  ...
  if (range.startDate || range.endDate) { if (!range.startDate || !range.endDate) throw new Error("A historical range requires both a start date and an end date."); params.set("start", range.startDate); params.set("end", range.endDate); }
