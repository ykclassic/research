...
function storeCsrf(token: string | null): void {
  try { if (token) window.sessionStorage.setItem(CSRF_STORAGE_KEY, token); else window.sessionStorage.removeItem(CSRF_STORAGE_KEY); } catch { /* restricted storage */ }
}
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (init.method && init.method !== "GET") {
    const csrf = getStoredCsrf() ?? getCookie("mr_csrf");
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new ApiError(`The application server did not respond within ${REQUEST_TIMEOUT_MS / 1000} seconds. Check the API deployment and try again.`, 0);
    }
...
export async function register(email: string, password: string): Promise<User> {
  return request<User>("/api/auth/register", { method: "POST", body: JSON.stringify({ email, password }) });
}
export async function login(email: string, password: string): Promise<User> {
  const user = await request<User>("/api/auth/login", { method: "POST", body: JSON.stringify({ email, password }) });
  await requestCsrfToken();
  return user;
}
export async function requestPasswordReset(email: string): Promise<string> {
  const response = await request<{ message: string }>("/api/auth/password-reset/request", { method: "POST", body: JSON.stringify({ email }) });
  return response.message;
}
export async function confirmPasswordReset(accessToken: string, password: string): Promise<string> {
  const response = await request<{ message: string }>("/api/auth/password-reset/confirm", { method: "POST", body: JSON.stringify({ access_token: accessToken, password }) });
  return response.message;
}
...
export async function getQuotes(symbols: string[], refresh = false): Promise<Quote[]> {
  const params = new URLSearchParams({ symbols: symbols.join(","), refresh: String(refresh) });
  const body = await request<{ quotes: Quote[] }>(`/api/market/quotes?${params}`);
  return body.quotes;
}
export async function getWatchlists(): Promise<Watchlist[]> {
  const body = await request<{ watchlists: Watchlist[] }>("/api/watchlists"); return body.watchlists;
}
export async function createWatchlist(name: string): Promise<Watchlist> {
  return authenticatedMutation<Watchlist>("/api/watchlists", { method: "POST", body: JSON.stringify({ name }) });
}
export async function renameWatchlist(id: string, name: string): Promise<Watchlist> {
  return authenticatedMutation<Watchlist>(`/api/watchlists/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ name }) });
}
export async function deleteWatchlist(id: string): Promise<void> {
  await authenticatedMutation<void>(`/api/watchlists/${encodeURIComponent(id)}`, { method: "DELETE" });
}
export async function addWatchlistSymbol(id: string, symbol: string): Promise<WatchlistItem> {
  return authenticatedMutation<WatchlistItem>(`/api/watchlists/${encodeURIComponent(id)}/symbols`, { method: "POST", body: JSON.stringify({ symbol }) });
}
export async function removeWatchlistSymbol(id: string, symbol: string): Promise<void> {
  await authenticatedMutation<void>(`/api/watchlists/${encodeURIComponent(id)}/symbols/${encodeURIComponent(symbol)}`, { method: "DELETE" });
}
