...
export type ResearchWorkspace = { id:string; user_id:string; name:string; workspace_type:string; description:string; shared:boolean; created_at:string; updated_at:string };
export type ResearchWorkspaceSnapshot = { workspace:ResearchWorkspace; assets:Array<Record<string,any>>; dashboards:Array<Record<string,any>>; links:Array<Record<string,any>>; scorecards:Array<Record<string,any>>; automation:Array<Record<string,any>> };
export async function getResearchWorkspaces():Promise<ResearchWorkspace[]>{return (await request<{items:ResearchWorkspace[]}>("/api/research-workspaces")).items;}
export async function createResearchWorkspace(payload:{name:string;workspace_type:string;description?:string;shared?:boolean}):Promise<ResearchWorkspace>{return (await authenticatedMutation<{item:ResearchWorkspace}>("/api/research-workspaces",{method:"POST",body:JSON.stringify(payload)})).item;}
export async function deleteResearchWorkspace(id:string):Promise<void>{await authenticatedMutation<void>(`/api/research-workspaces/${encodeURIComponent(id)}`,{method:"DELETE"});}
export async function getResearchWorkspace(id:string):Promise<ResearchWorkspaceSnapshot>{return request<ResearchWorkspaceSnapshot>(`/api/research-workspaces/${encodeURIComponent(id)}`);}
export async function addResearchWorkspaceAsset(id:string,payload:{symbol:string;role:string}):Promise<Record<string,any>>{return (await authenticatedMutation<{item:Record<string,any>}>(`/api/research-workspaces/${encodeURIComponent(id)}/assets`,{method:"POST",body:JSON.stringify(payload)})).item;}
export async function removeResearchWorkspaceAsset(id:string,symbol:string):Promise<void>{await authenticatedMutation<void>(`/api/research-workspaces/${encodeURIComponent(id)}/assets/${encodeURIComponent(symbol)}`,{method:"DELETE"});}
export async function saveResearchWorkspaceDashboard(id:string,payload:Record<string,any>):Promise<Record<string,any>>{return (await authenticatedMutation<{item:Record<string,any>}>(`/api/research-workspaces/${encodeURIComponent(id)}/dashboards`,{method:"POST",body:JSON.stringify(payload)})).item;}
export async function createResearchScorecard(id:string,payload:Record<string,any>):Promise<Record<string,any>>{return (await authenticatedMutation<{item:Record<string,any>}>(`/api/research-workspaces/${encodeURIComponent(id)}/scorecards`,{method:"POST",body:JSON.stringify(payload)})).item;}
export async function evaluateResearchScorecard(workspaceId:string,scorecardId:string,symbol:string):Promise<Record<string,any>>{return (await authenticatedMutation<{item:Record<string,any>}>(`/api/research-workspaces/${encodeURIComponent(workspaceId)}/scorecards/${encodeURIComponent(scorecardId)}/evaluate`,{method:"POST",body:JSON.stringify({symbol})})).item;}
export async function createResearchAutomation(id:string,payload:Record<string,any>):Promise<Record<string,any>>{return (await authenticatedMutation<{item:Record<string,any>}>(`/api/research-workspaces/${encodeURIComponent(id)}/automation`,{method:"POST",body:JSON.stringify(payload)})).item;}
