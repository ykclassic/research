...
export interface SignalOutcomeRecord {
  signal_id: string; symbol: string; signal: SignalDirection; score: number; confidence: number;
  dispatched_at: string; entry_price: number; stop_loss: number | null; target_price: number | null;
  provider: string; timeframe: string; signal_engine_version: string; calculated_at: string; latest_candle_timestamp: string;
  target_tagged_at: string | null; stop_tagged_at: string | null;
  target_tag_latency_seconds: number | null; stop_tag_latency_seconds: number | null;
  first_touch_price: number | null; first_touch_timestamp: string | null; outcome: SignalOutcomeStatus;
  observed_at: string | null; observation_candle_timestamp: string | null; observation_source: string | null; coverage_warning: string | null;
}
...
export async function logSignalOutcome(signal: CryptoSignal, dispatchedAt?: string): Promise<SignalOutcomeRecord> { return authenticatedMutation<SignalOutcomeRecord>("/api/signal-outcomes/log", { method: "POST", body: JSON.stringify({ signal, dispatched_at: dispatchedAt }) }); }
