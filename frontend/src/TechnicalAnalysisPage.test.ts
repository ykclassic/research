...
describe("chart range state", () => {
  ...
  it("uses no local market-data filtering for the recent API window", () => {
  ...
  it("serializes custom dates as UTC server query boundaries", () => {
...
describe("canonical API response validation", () => {
  it("accepts a matching timeframe and symbol", () => {
    expect(validateAnalysisResponse(BASE, "BTC/USD", "1h")).toBe(BASE);
  });

  it("rejects a mismatched timeframe so stale data cannot be displayed", () => {
