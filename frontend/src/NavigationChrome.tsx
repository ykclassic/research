...
export default function NavigationChrome() {
  const [user, setUser] = useState<User | null>(null); const [mobileOpen, setMobileOpen] = useState(false); const [page, setPage] = useState(currentPage);
  ...
  return <><header className="app-chrome-header"><button className="chrome-menu" type="button" onClick={() => setMobileOpen(value => !value)} aria-label="Toggle navigation">{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button><div className="chrome-heading"><div className="eyebrow">Adaptive Intelligence</div><strong>{activeItem.label}</strong></div><div className="chrome-user"><span>{user.email}</span><button type="button" className="chrome-signout" onClick={() => void signOut()}><LogOut size={15} /> Sign out</button></div></header><aside className={`app-sidebar ${mobileOpen ? "open" : ""}`} aria-label="Primary navigation"><div className="sidebar-brand"><div className="brand-mark"><BrainCircuit size={20} /></div><div><div className="eyebrow">Adaptive</div><strong>Market Research</strong></div></div><nav className="sidebar-nav">{GROUPS.map(group => <div className="nav-group" key={group.label}><div className="nav-group-label">{group.label}</div>{group.items.map(item => { const Icon = item.icon; const active = page === item.page; const disabled = item.page === "settings"; return <button key={item.page} type="button" className={`sidebar-item ${active ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={() => navigate(item)} disabled={disabled} title={disabled ? "Coming soon" : undefined}><Icon size={17} /><span>{item.label}</span>{disabled ? <span className="coming-soon">Soon</span> : active ? <ChevronRight size={15} /> : null}</button>; })}</div>)}</nav><div className="sidebar-footer"><ShieldCheck size={15} /><span>Validated research workspace</span></div></aside>{mobileOpen && <button className="sidebar-backdrop" type="button" aria-label="Close navigation" onClick={() => setMobileOpen(false)} />}</>;
}
