...
export default function NavigationChrome() {
  ...
  const syncFromLocation = useCallback(() => {
    ...
    setPage(next);
    if (next === "settings") { window.history.replaceState({}, "", "/dashboard"); setPage("market"); return; }
    let attempts = 0;
    const timer = window.setInterval(() => { attempts += 1; if (clickLegacyNavigation(next) || attempts >= 50) window.clearInterval(timer); }, 100);
  }, []);
  ...
  return <>
    <header className="app-chrome-header">
      <button className="chrome-menu" type="button" onClick={() => setMobileOpen(value => !value)} aria-label="Toggle navigation">{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button>
    ...
    <aside className={`app-sidebar ${mobileOpen ? "open" : ""}`} aria-label="Primary navigation">
      <div className="sidebar-brand"><div className="brand-mark"><BrainCircuit size={20} /></div><div><div className="eyebrow">Adaptive</div><strong>Market Research</strong></div></div>
      <nav className="sidebar-nav">
        {GROUPS.map(group => <div className="nav-group" key={group.label}>
          <div className="nav-group-label">{group.label}</div>
          {group.items.map(item => { const Icon = item.icon; const active = page === item.page; const disabled = item.page === "settings"; return <button key={item.page} type="button" className={`sidebar-item ${active ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={() => navigate(item)} disabled={disabled} title={disabled ? "Coming soon" : undefined}><Icon size={17} /><span>{item.label}</span>{disabled ? <span className="coming-soon">Soon</span> : active ? <ChevronRight size={15} /> : null}</button>; })}
        </div>)}
      </nav>
